...
class MY_NET(nn.Module):
    ...
    def forward(self,x1,x2):
        ...
        """sub_branch"""
        sub_layer1_ = torch.abs(x1_layer1 - x2_layer1)  # 逐元素做差，然后取绝对值。[4,64,64,64]

        sub_layer2_ = torch.abs(x1_layer2 - x2_layer2)  # [4,128,32,32]

        sub_layer3_ = torch.abs(x1_layer3 - x2_layer3)  # [4,256,16,16]

        sub_layer4_ = torch.abs(x1_layer4 - x2_layer4)  # [4,512,8,8]

        up4 = self.up4(sub_layer4_)  # [4,256,16,16]
        out3 = sub_layer3_ + up4  # [4,256,16,16]
        up3 = self.up3(out3)  # [4,128,32,32]
        out2 = sub_layer2_ + up3  ##[4,128,32,32]
        up2 = self.up2(out2)  # [4,64,64,64]

        out1 = sub_layer1_ + up2  # [4,64,64,64]

        output  = self.output(out1)  # [4,2,64,64]
